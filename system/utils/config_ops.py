...
from system.data.image import Image
from system.data.utils.image import (
    find_image_files_folder_per_class,
    find_image_files_from_file,
)
from system.utils.custom import save_local_model
from system.utils.request_util import get_dataset, get_split
    ...
    try:
        if option == "option1":
            ...
            info_file = [
                ...
                if f.startswith("labels.") or f.startswith("train.")
            ]
...
def default_model(local_sess, session, USER_ROOT, appConfig):
    ...
    data_dir = os.path.join("system", "default", "iris")
